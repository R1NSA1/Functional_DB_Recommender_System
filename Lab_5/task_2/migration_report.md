...
Для больших файлов рейтингов используется bulk-загрузка через PostgreSQL `COPY` во временную таблицу `tmp_ratings_raw`. После этого пользователи и рейтинги переносятся в основные таблицы SQL-запросами. Такой подход быстрее и надежнее, чем построчные `INSERT`.
...
## Результат полной загрузки

Полная загрузка выполнена на PostgreSQL `127.0.0.1:5433`, база `recsys_lab5`, источники `ratings.csv` и `links.csv`.
...
| `movie_genres` | 91015 |
| `keywords` | 19949 |
| `movie_keywords` | 156598 |
| `people` | 353343 |
| `movie_cast` | 562044 |
| `movie_crew` | 463836 |
| `content_features` | 45433 |

Из полного `ratings.csv` пропущено 43692 рейтинга, потому что для них не нашлось сопоставленного фильма в загруженном каталоге.
